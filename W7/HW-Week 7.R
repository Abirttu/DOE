...
gad(model)
# Hypothesis:
# H0:tau_i = 0 for all i
# Ha:tau_i != 0 for some i
# Model Equation:
# y_ij=mu+tau_i+beta_j+e_ij
# where mu = grand mean, tau_i = Chemical effect,
# beta_j = Bolt (block), e_ij = random error ~ N(0,sigma^2)
# P value (0.1211) > alpha (0.05) hence we fail to reject H0.
# There is no difference among the four chemical agents at alpha = 0.05 level.
# Problem 4.16
# Calculate model parameters tau_i:
# Formula: tau_i = mu_i-mu
obs <- c(73,68,74,71,67,73,67,75,72,70,75,68,78,73,68,73,71,75,75,69)
mean(obs)
...
tau_4
# tau_1 = -1.15, tau_2 = -0.35, tau_3 = 0.65, tau_4 = 0.85
...
# Hypothesis 
# H0: tau_i = 0 for all i
# Ha: tau_i != 0 for some i, i=1,2,3,4,5
# Model equation: X_ijk = mu+tau_i+beta_j+alpha_k+e_ijk
# where, mu=grand mean, tau_i=Ingredients effect, beta_j = Batch (block 1)
# alpha_k = Day (block 2), e_ijk = random error ~N(0,sigma^2)
# Conclusions:
# Hence the p value (0.000488) of ingredients is highly significant and less than
# alpha=0.05 so we reject the null hypothesis H0.
